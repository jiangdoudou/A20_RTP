...
S: Sysfs=/devices/platform/sunxi-ts/input/input2
U: Uniq=
H: Handlers=event2 
B: PROP=0
B: EV=b
...
root@ITEAD:/home# 
root@ITEAD:/home# 
root@ITEAD:/home# cat /proc/bus/input/devices 
I: Bus=0019 Vendor=0001 Product=0001 Version=0100
N: Name="sunxi-ir"
P: Phys=RemoteIR/input1
S: Sysfs=/devices/virtual/input/input0
...
H: Handlers=event2 
*/
////make ARCH=arm CROSS_COMPILE=./external-toolchain/bin/arm-linux-gnueabi-  -j8
///devices/platform/sunxi-ts/input/input2


//////////////////add by jiangdou/////////////
#if 1

#define print_dou(format,args...)     printk(KERN_ERR "[spi-err] "format,##args)
...
static struct sunxi_ts_data *sunxi_ts_data_alloc(struct platform_device *pdev)
{
	...
	input_set_abs_params(ts_data->input, ABS_MT_TOUCH_MAJOR, 0, 1, 0, 0);
...
static int __devinit sunxi_ts_probe(struct platform_device *pdev)
{
	...
	//IRQ handler type mismatch for IRQ 40
	//
	err = request_irq(irq, sunxi_isr_tp,IRQF_DISABLED, pdev->name, pdev);
	if (err) {
...
*/
	...
	print_dou("sunxi-ts: tp_screen..jiangdou....\n");
 	//if (SCIRPT_ITEM_VALUE_TYPE_INT  != type) {
	//	pr_err("%s: type err  rtp_used = %d. \n", __func__, val.val);
	//	goto script_get_err;
	//}
	//device_used = val.val;//==rtp_used =1
	device_used = 1;//==rtp_used =1
	if (1 == device_used) {
	...
	//	if(SCIRPT_ITEM_VALUE_TYPE_INT  != type){
	 //       pr_err("sunxi_ts_init: script_get err. \n");
	  //      goto script_get_err;
		//}
		//tp_screen_size = val.val;//==rtp_screen_size
		tp_screen_size = 7;//==rtp_screen_size
